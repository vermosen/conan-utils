...
class BoostConan(ConanFile):
    ...
    def requirements(self):
        if self.zip_bzip2_requires_needed:
            ...
            if self.options.zstd:
                self.requires("zstd/1.3.@%s/%s" % (self.user, self.channel))
        if  self.icu_requires_needed:
            self.requires("icu/64.2@%s/%s" % (self.user, self.channel))
    ...
    def source(self):
        if tools.os_info.is_windows:
            sha256 = "8c20440aaba21dd963c0f7149517445f50c62ce4eb689df2b5544cc89e6e621e"
            extension = ".zip"
        else:
    ...
    def _b2_binary_format(self):
        return {"Windows": "pe",
                "WindowsStore": "pe",
                "Linux": "elf"}.get(str(self.settings.os))
