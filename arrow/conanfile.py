...
class ArrowConan(ConanFile):
    ...
    default_options = {
        ...
        "with_snappy": False,
        "with_zlib": True,
        "with_zstd": False, # error when turned on
    }
    ...
    def requirements(self):
        ...
        if self.options.with_lz4:
            self.requires("lz4/[>=1.9.2]@%s/%s" % (self.user, self.channel))
        if self.options.with_snappy:
            self.requires("snappy/1.1.8")
        if self.options.with_zlib:
            self.requires("zlib/1.2.11@%s/%s" % (self.user, self.channel))
        if self.options.with_zstd:
            self.requires("zstd/[>=1.4.5]@%s/%s" % (self.user, self.channel))
        if self._with_re2():
    ...
    def _configure_cmake(self):
        ...
        self._cmake.definitions["ARROW_HDFS"] = self.options.hdfs_bridge
        ...
        self._cmake.configure(source_folder="%s/cpp" % self._source_subfolder)
        return self._cmake
